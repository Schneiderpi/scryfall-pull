...
@click.command()
@click.option("-o", "--out", type=click.Path(dir_okay=False,writable=True),help="Filename to store output. Output will be in csv and file WILL BE OVERWRITTEN") #TODO
...
def pull(out,search,input,columns,column_file,max,image):
    ...
    if out is None:
        ...
        with open(out, 'w') as f:
            ...
            #Header
            writer.writerow(cards[0].get_card().keys())

            for card in tqdm(cards):
                writer.writerow(card.get_card().values())
